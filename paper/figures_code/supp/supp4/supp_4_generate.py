...
M = np.array([[alpha_0, beta_0], [alpha_1, beta_1]])
...
save_path = os.path.abspath(
    os.path.join(sys.path[0], "../../../figures/supp/supp4/data")
)
    ...
    for seq in seqs:
        ...
        for tidx in range(num_trees):
            ...
            this_save_path = os.path.join(
                save_folder, str(tidx), str(seq), "replay_data"
            )
            ...
            np.save(
                os.path.join(this_save_path, "replay_history.npy"),
                np.asarray(replays, dtype="object"),
            )
